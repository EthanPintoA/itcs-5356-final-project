...
from ecg5000_dataloader import load_pickle_data
...
def ecg5000_pickle():
    ...
    train_data, val_data = load_pickle_data(train_path, val_path)

    train_y, train_X = train_data[:, 0], train_data[:, 1:]
    val_y, val_X = val_data[:, 0], val_data[:, 1:]

    # Pad sequences to 1250 timesteps
    train_X = pad_sequences(train_X, maxlen=1250, padding='post', dtype='float32')
    val_X = pad_sequences(val_X, maxlen=1250, padding='post', dtype='float32')

    # Reshape data to add the feature dimension
    train_X = train_X.reshape(len(train_X), 1250, 1)
    val_X = val_X.reshape(len(val_X), 1250, 1)
    ...
    # Evaluate the model
    pred = model.predict(val_X).argmax(axis=1)
    print(classification_report(val_y, pred, digits=4))
